...
export const AccessibilityLabels = {
  ...
  semantic: {
    ...
    input: (label: string, required?: boolean) => `${label}${required ? ' (obrigatório)' : ''}`,
  },
...
export const EmotionalMessages = {
  ...
  // Por conquista
  achievements: {
    habitStreak: (days: number) => `🔥 ${days} dias consecutivos! Você é incrível!`,
    emotionCheckIn: (days: number) => `💙 ${days} dias cuidando de você. Continue assim!`,
    firstPost: () => `🎉 Bem-vinda à comunidade! Sua voz importa.`,
  },
};
```
